...
out_file="$out_dir/${work}-shot-cards.mp4"
mkdir -p "$out_dir"
...
while IFS=$'\t' read -r shot_id duration caption camera action narration; do
  ...
  ffmpeg -hide_banner -loglevel error -y \
    -f lavfi -i "color=c=0x041E42:s=1280x720:d=${duration}:r=24" \
    -vf "drawtext=textfile=$card_text:fontcolor=white:fontsize=32:line_spacing=10:x=70:y=70,format=yuv420p" \
    -c:v libx264 \
    -pix_fmt yuv420p \
    -t "$duration" \
    "$clip_file"
