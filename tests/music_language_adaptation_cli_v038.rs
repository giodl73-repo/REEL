...
fn authority(id: &str, digit: char, status: &str, with_decision: bool) -> AuthorityRef {
    AuthorityRef {
    ...
    let manifest = LanguageAdaptation {
        ...
        translation_links: vec![
            TranslationLink {
                ...
                rationale: "Synthetic two-unit opening link for contract testing only.".into(),
            },
            TranslationLink {
                id: "closing-link".into(),
                source_unit_ids: vec!["s3".into(), "s4".into()],
                target_unit_ids: vec!["t3".into(), "t4".into(), "t5".into()],
                rationale: "Synthetic unequal closing link exercises the prosody ledger.".into(),
            },
        ],
        ...
        prosody_exceptions: vec![ProsodyException {
            ...
            note_ids: vec!["note-4".into()],
            rationale: "The synthetic target uses one additional unit on the closing note.".into(),
...
fn write(path: &Path, manifest: &LanguageAdaptation) {
    fs::write(path, serde_yaml::to_string(manifest).unwrap()).unwrap();
}

#[test]
fn cli_validates_complete_same_music_language_adaptation() {
    let temporary = tempdir().unwrap();
    let adaptation = build_fixture(temporary.path());
    let output = Command::new(env!("CARGO_BIN_EXE_reel"))
        .arg("music-language-adaptation-check")
        .arg(&adaptation)
        .args(["--output", "json"])
        .output()
        .unwrap();
    ...
    assert_eq!(report["source_units"], 4);
    assert_eq!(report["target_units"], 5);
    assert_eq!(report["underlay_events"], 5);
    assert_eq!(report["prosody_exceptions"], 1);
    assert_eq!(report["complete_model_inheritance"], true);
    assert_eq!(report["exact_accompaniment_duration"], true);
    assert_eq!(report["shareable"], false);
}

#[test]
fn rejects_text_model_and_accompaniment_tampering() {
    let temporary = tempdir().unwrap();
    let path = build_fixture(temporary.path());
    fs::write(temporary.path().join("target.txt"), "changed text\n").unwrap();
    assert!(reel_music::language_adaptation::validate(&path).is_err());

    let path = build_fixture(&temporary.path().join("model"));
    let mut manifest = reel_music::language_adaptation::load(&path).unwrap();
    manifest.preserved_model_targets.pop();
    write(&path, &manifest);
    assert!(reel_music::language_adaptation::validate(&path).is_err());

    let path = build_fixture(&temporary.path().join("duration"));
    let mut manifest = reel_music::language_adaptation::load(&path).unwrap();
    manifest.accompaniment.timebase.samples_per_channel -= 1;
    write(&path, &manifest);
    assert!(reel_music::language_adaptation::validate(&path).is_err());
}

#[test]
fn rejects_incomplete_translation_underlay_and_prosody_governance() {
    let temporary = tempdir().unwrap();
    let path = build_fixture(temporary.path());
    let mut manifest = reel_music::language_adaptation::load(&path).unwrap();
    manifest.translation_links[1].target_unit_ids.pop();
    write(&path, &manifest);
    assert!(reel_music::language_adaptation::validate(&path).is_err());

    let path = build_fixture(&temporary.path().join("underlay"));
    let mut manifest = reel_music::language_adaptation::load(&path).unwrap();
    manifest.underlay[4].note_ids[0] = "missing-note".into();
    write(&path, &manifest);
    assert!(reel_music::language_adaptation::validate(&path).is_err());

    let path = build_fixture(&temporary.path().join("prosody"));
    let mut manifest = reel_music::language_adaptation::load(&path).unwrap();
    manifest.prosody_exceptions.clear();
    write(&path, &manifest);
    assert!(reel_music::language_adaptation::validate(&path).is_err());

    let path = build_fixture(&temporary.path().join("prosody-link"));
    let mut manifest = reel_music::language_adaptation::load(&path).unwrap();
    manifest.prosody_exceptions[0].translation_link_id = "missing-link".into();
    write(&path, &manifest);
    assert!(reel_music::language_adaptation::validate(&path).is_err());

    let path = build_fixture(&temporary.path().join("prosody-review"));
    let mut manifest = reel_music::language_adaptation::load(&path).unwrap();
    manifest.prosody_exceptions[0].required_review_roles.clear();
    write(&path, &manifest);
    assert!(reel_music::language_adaptation::validate(&path).is_err());

    let path = build_fixture(&temporary.path().join("authority"));
    let mut manifest = reel_music::language_adaptation::load(&path).unwrap();
    manifest.target_text.authority.status = "not-reviewed".into();
    manifest.target_text.authority.decision_refs.clear();
    write(&path, &manifest);
    assert!(reel_music::language_adaptation::validate(&path).is_err());
}
