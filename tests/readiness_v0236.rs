...
#[test]
fn preserves_legacy_and_embedded_asset_manifests_as_ready() {
    let report = validate(
  ...
  - id: second
    ...
    animation:
      timing_fps: 24
      frames:
        - { asset: second.png, hold_frames: 24 }
"#,
    );
...
#[test]
fn rejects_selected_media_without_a_materialized_source() {
    let dir = tempdir().unwrap();
    let path = dir.path().join("manifest.yaml");
    fs::write(
        &path,
...
"#,
    ...
    let error = reel::production::validate(&reel::production::load(path).unwrap())
        .unwrap_err()
        .to_string();
    assert!(error.contains("declares selected media without a materialized picture source"));
}
