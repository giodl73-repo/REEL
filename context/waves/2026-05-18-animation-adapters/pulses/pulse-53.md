# Pulse 53: Artifact source totals

## Outcome

Added aggregate verified source manifest paths to batch artifact and review
reports.

## Changes

- `artifact-check-all` JSON now includes `source_manifests`, a sorted list of
  verified source manifest paths across checked works.
- `artifact-check-all` text output now includes `sources=<count>` beside works
  and artifact totals.
- `review-all` JSON now includes aggregate `source_manifests`.
- `review-all` Markdown now includes source manifests in aggregate verification
  totals.
