    ...
    pub fn ffprobe_json(&self, path: &Path) -> Result<String> {
        self.run_external(
            ...
            &[
                ...
                "error".to_string(),
                "-show_entries".to_string(),
                "format=duration:stream=index,codec_type,codec_name,width,height,sample_aspect_ratio,pix_fmt,r_frame_rate,avg_frame_rate,duration,nb_frames:stream_tags=rotate:stream_side_data=rotation".to_string(),
                "-of".to_string(),
                "json".to_string(),
            ],
            &[self.path_argument(path)?],
